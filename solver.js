...
function displayPath(path) {
    ...
    for (var i = 0; i < n; i++) {
        if(i<path.length-1){
            dist1[i] = (d1[path[i]] [path[i+1]]);
        //    console.log();
        //    graph.dist.add(d1[path[i]] [path[i+1]]);
        }
        if(i === path.length-1){
            dist1[i] = d1[path[i]][path[0]];
        }
        graph.edges.add({from: path[i], to: path[(i+1 ) % n] , label: String(Math.floor(1*dist1[i])) });
...
function displayState(state) {
    ...
    info.innerHTML = '<b>Distance:</b> ' + state.length.toFixed(3) + '<br>' +
        // '<b>Approximation:</b> ' + state.approximation.toFixed(3) + '<br>' +
        '<b>Route:  &nbsp  &nbsp  &nbsp</b> ' + state.path.join(' -> ');
}
